...
class RamachandranComponent extends PolymerElement {
    ...
    public createChart() {
        ...
        d3.select('#rama-info-chains').on("mouseover", () =>{return tooltip.style("visibility", "visible")
            .style('opacity', .95)
    ...
    public updateChart(chainsToShow: any[], ramaContourPlotType: number, modelsToShow: number[]) {
        ...
        function filterModels() {
            let residues = [];
            RamachandranComponent.molecules.forEach((molecule: any) => {
                molecule.chains.forEach((chain: any) => {
        ...
        let filteredResidues = filterModels().sort((a: any, b: any) => {
